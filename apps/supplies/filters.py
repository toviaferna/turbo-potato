...
class LibroCompraFilter(FilterSet):
    fecha_desde = django_filters.DateFilter(widget=DateInput(attrs={'placeholder': '1970-01-01'}), field_name='fechaDocumento', lookup_expr='gte', label='Desde')
    fecha_hasta = django_filters.DateFilter(widget=DateInput(), field_name='fechaDocumento', lookup_expr='lte', label='Hasta')
    ...
    class Meta:
        model = Compra
        fields = ["fecha_desde","fecha_hasta","comprobante","proveedor","deposito","es_credito","es_vigente"]