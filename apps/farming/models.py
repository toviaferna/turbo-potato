...
class Finca(models.Model):
    descripcion = models.CharField(
        max_length=200, verbose_name="Descripción", unique=True
    )
    dimension_ha = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Dimension Ha"
    )
...
class TipoActividadAgricola(models.Model):
    ...
    descripcion = models.CharField(
        max_length=200, verbose_name="Descripcion", unique=True
    )
...
class Zafra(models.Model):
    ...
    kg_estimado = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Kg Estimado"
    )
...
class Lote(models.Model):
    ...
    dimension = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Dimensión HA"
    )
    created = models.DateTimeField(auto_now_add=True)
...
class MaquinariaAgricola(models.Model):
    descripcion = models.CharField(max_length=200, verbose_name="Descripcion")
    tipo_maquinaria_agricola = models.ForeignKey(
        TipoMaquinariaAgricola,
        on_delete=models.DO_NOTHING,
        verbose_name="Maquinaria Agrícola Tipo",
    )
...
class PlanActividadZafra(models.Model):
    ...
    zafra = models.ForeignKey(
        Zafra, on_delete=models.DO_NOTHING, null=True, blank=True, verbose_name="Zafra"
    )
    observacion = models.CharField(
        max_length=300, null=True, blank=True, verbose_name="Observación"
    )
...
class PlanActividadZafraDetalle(models.Model):
    plan_actividad_zafra = models.ForeignKey(
        PlanActividadZafra, on_delete=models.DO_NOTHING
    )
    fecha_actividad = models.DateField(verbose_name="Fecha Act.")
    finca = models.ForeignKey(Finca, on_delete=models.DO_NOTHING, verbose_name="Finca")
    tipo_actividad_agricola = models.ForeignKey(
        TipoActividadAgricola,
        on_delete=models.DO_NOTHING,
        null=True,
        blank=True,
        verbose_name="Tipo Actividad Agrícola",
    )
    descripcion = models.CharField(
        max_length=300, null=True, blank=True, verbose_name="Descripción"
    )
    costo = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Costo Estimado"
    )
...
class Acopio(models.Model):
    ...
    deposito = models.ForeignKey(
        Deposito, on_delete=models.DO_NOTHING, verbose_name="Depósito"
    )
    conductor = models.ForeignKey(
        Persona, on_delete=models.DO_NOTHING, verbose_name="Conductor"
    )
    camion = models.ForeignKey(
        MaquinariaAgricola, on_delete=models.DO_NOTHING, verbose_name="Camión"
    )
    fecha = models.DateField(verbose_name="Fecha")
    comprobante = models.CharField(max_length=30, verbose_name="Comprobante")
    peso_bruto = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Peso Bruto", default=0
    )
    peso_tara = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Peso Tara", default=0
    )
    peso_descuento = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Peso Desc.", default=0
    )
    peso_bonificacion = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Peso Bonif.", default=0
    )
    es_transportadora_propia = models.BooleanField(
        verbose_name="Es Transportadora Propia?", default=False
    )
...
class AcopioCalificacion(models.Model):
    acopio = models.ForeignKey(Acopio, on_delete=models.DO_NOTHING)
    calificacion_agricola = models.ForeignKey(
        CalificacionAgricola,
        on_delete=models.DO_NOTHING,
        verbose_name="Calif. Agrícola",
    )
    grado = models.DecimalField(max_digits=15, decimal_places=2, verbose_name="Grado")
    porcentaje = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Porcentaje"
    )
    peso = models.DecimalField(max_digits=15, decimal_places=2, verbose_name="Peso")
...
class ActividadAgricola(models.Model):
    tipo_actividad_agricola = models.ForeignKey(
        TipoActividadAgricola,
        on_delete=models.DO_NOTHING,
        verbose_name="Tipo Act. Agrícola",
    )
    lote = models.ForeignKey(Lote, on_delete=models.DO_NOTHING, verbose_name="Lote")
    zafra = models.ForeignKey(Zafra, on_delete=models.DO_NOTHING, verbose_name="Zafra")
    finca = models.ForeignKey(Finca, on_delete=models.DO_NOTHING, verbose_name="Finca")
    ...
    empleado = models.ForeignKey(
        Persona, on_delete=models.DO_NOTHING, verbose_name="Empleado"
    )
    fecha_hora_registro = models.DateTimeField(
        auto_now_add=True, verbose_name="Fecha Hora Registro"
    )
    observacion = models.CharField(
        max_length=300, null=True, blank=True, verbose_name="Observación"
    )
    es_vigente = models.BooleanField(verbose_name="Vigente?", default=True)
    es_servicio_contratado = models.BooleanField(
        verbose_name="Es contratado?", default=False
    )
    cantidad_trabajada = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="HA Trabajada"
    )
    ...
    def total_maquinaria(self):
        retorno = sum(
            round(x.precio * x.ha_trabajada)
            for x in self.actividadagricolamaquinariadetalle_set.all()
        )
        if retorno is None:
            retorno = 0
        return retorno
    ...
    def total_item(self):
        retorno = sum(
            round(x.costo * x.cantidad)
            for x in self.actividadagricolaitemdetalle_set.all()
        )
        if retorno is None:
            retorno = 0
        return retorno
...
class ActividadAgricolaMaquinariaDetalle(models.Model):
    actividad_agricola = models.ForeignKey(
        ActividadAgricola, on_delete=models.DO_NOTHING
    )
    maquinaria = models.ForeignKey(
        MaquinariaAgricola, on_delete=models.DO_NOTHING, verbose_name="Maquinaria"
    )
    ha_trabajada = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Ha Trabajada"
    )
...
class ActividadAgricolaItemDetalle(models.Model):
    ...
    cantidad = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Cantidad"
    )
    porcentaje_impuesto = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="% Impuesto"
    )
    dosis = models.DecimalField(max_digits=15, decimal_places=2, verbose_name="Dosis")
    deposito = models.ForeignKey(
        Deposito, on_delete=models.DO_NOTHING, verbose_name="Deposito", default=1
    )
...
class Contrato(models.Model):
    ...
    persona = models.ForeignKey(
        Persona, on_delete=models.DO_NOTHING, verbose_name="Persona"
    )
    costo_pactado = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Precio Pactado"
    )
    fecha = models.DateField(verbose_name="Fecha")
...
class LiquidacionAgricola(models.Model):
    ...
    proveedor = models.ForeignKey(
        Persona, on_delete=models.DO_NOTHING, verbose_name="Proveedor"
    )
    fecha_hora_registro = models.DateTimeField(
        auto_now_add=True, verbose_name="Fecha Hora Registro"
    )
    ...
    precio_unitario = models.DecimalField(
        max_digits=15, decimal_places=0, verbose_name="Precio"
    )
    tipo = models.CharField(
        max_length=50, choices=VALORESENUMTIPMOV, verbose_name="Tipo Liquidación"
    )
    ...
    def total(self):
        return sum(
            round(self.precio_unitario * x.cantidad)
            for x in self.liquidacionagricoladetalle_set.all()
        )


class LiquidacionAgricolaDetalle(models.Model):
    liquidacion_agricola = models.ForeignKey(
        LiquidacionAgricola, on_delete=models.DO_NOTHING
    )
    cantidad = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Cantidad"
    )
...
class CierreZafra(models.Model):
    ...
    def total_acopiado(self):
        return sum(
            round(x.cantidad_acopio_neto) for x in self.cierrezafradetalle_set.all()
        )
...
class CierreZafraDetalle(models.Model):
    ...
    finca = models.ForeignKey(Finca, on_delete=models.DO_NOTHING, verbose_name="Finca")
    ha_cultivada = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="HA Cultivada"
    )
    cantidad_acopio_neto = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="KG Acopiado"
    )
    rendimiento = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Rendimiento"
    )
    costo_total = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Costo Total"
    )
    costo_ha = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Costo HA"
    )
    costo_unitario = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Costo Unit."
    )
